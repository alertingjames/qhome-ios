...
class MyOrders2ViewController: BaseViewController, UITableViewDataSource, UITableViewDelegate  {
    ...
    override func viewDidLoad() {
        ...
        var tap = UITapGestureRecognizer(target: self, action: #selector(self.toAll(_:)))
        self.view_all.addGestureRecognizer(tap)
        
        tap = UITapGestureRecognizer(target: self, action: #selector(self.toPlaced(_:)))
    ...
    @objc func toAll(_ sender: UITapGestureRecognizer? = nil) {
    ...
    func selectedTab(selStatus:Int){
        resetTabs()
        if selStatus == 0{
            self.lbl_placed.attributedText = NSAttributedString(string: Language().placed, attributes: selAttrs)
    ...
    func getOrders(status:Int){
        ...
        APIs.getUserOrderItems(member_id: thisUser.idx, handleCallback: {
            ...
            if result_code == "0"{
                ...
                gMyOrdersViewController.getItemStatus(status: "placed", orderItems: orderItems!)
                gMyOrdersViewController.getItemStatus(status: "confirmed", orderItems: orderItems!)
                gMyOrdersViewController.getItemStatus(status: "prepared", orderItems: orderItems!)
                gMyOrdersViewController.getItemStatus(status: "ready", orderItems: orderItems!)
                gMyOrdersViewController.getItemStatus(status: "delivered", orderItems: orderItems!)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell:MyOrderCell2 = tableView.dequeueReusableCell(withIdentifier: "MyOrderCell2", for: indexPath) as! MyOrderCell2
        ...
        cell.lbl_status.text = self.orderItems[index].status
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        for order in gMyOrdersViewController.orders{
            if order.idx == item.orderId{
                gOrder = order
